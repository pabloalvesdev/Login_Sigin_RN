...
export default class Login extends Component {
  ...
  render() {
    ...
    <Text style={{alignSelf: 'flex-start', marginLeft: '23%', color: 'red'}}>Tentativas: {this.state.erros !== 0 ? this.state.erros: null}</Text>
        <View style={LoginStyle.buttonContainer}>
          <Button color={'black'} title={'Entrar'} onPress={()=>{
            if (this.state.user == 'user' && this.state.password == 'password@123') {
